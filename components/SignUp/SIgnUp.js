...
const SignUp = () => {
    ...
    const handleSignUp = async (email, password) => {
        try {
            ...
            const {error, session, user} = await supabase.auth.signUp({email: email, password: password});
            console.log("accessed")
            error ? console.log(error) : console.log("session is ", session, "user is ", user);
            let {errorr} = await supabase.from("test").insert({
